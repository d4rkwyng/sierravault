#!/usr/bin/env python3
"""
Comprehensive Developer/Studio Research Pipeline
...
Usage:
  python research_developers_pipeline.py --developer "Dynamix"
  python research_developers_pipeline.py --all
  python research_developers_pipeline.py --list developers.txt
"""
...
# Paths
RESEARCH_DIR = Path(__file__).parent / "research" / "developers"
...
KAGI_DELAY = 1.0
SCRAPER_DELAY = 0.3
MOBYGAMES_API_DELAY = 1.5  # MobyGames recommends 1 req/sec

# All developers with 3+ games to research
ALL_DEVELOPERS = [
    # Major Sierra studios
    "Sierra On-Line",
    "Dynamix",
    "Impressions Games",
    "Coktel Vision",
...
def get_vault_games_for_developer(name: str) -> list[dict]:
    """Find all games in vault developed by this studio."""
    games = []
    ...
    for md_file in GAMES_DIR.rglob("*.md"):
        try:
            ...
            # Check developer field in YAML
            dev_match = re.search(r'^developer:\s*["\']?([^"\'\n]+)', content, re.MULTILINE)
            if dev_match:
                dev = dev_match.group(1).lower().strip()
                if any(v in dev for v in variations):
...
def research_mobygames_api(name: str, output_dir: Path) -> int:
    """Use MobyGames API for structured company data."""
    if not MOBYGAMES_API_KEY:
        print("    Skipping MobyGames API (no key)")
        return 0

    files_created = 0
    api_base = "https://api.mobygames.com/v1"

    # Check if already have API data
    if list(output_dir.glob("mobygames_api*.json")):
        print("    Already have MobyGames API data")
        return 0

    try:
        # Search for companies matching this name
        print(f"    MobyGames API: Searching companies...")
        params = {"api_key": MOBYGAMES_API_KEY, "title": name}
        resp = httpx.get(f"{api_base}/games", params=params, timeout=30)

        if resp.status_code == 200:
            search_data = resp.json()
            games_found = search_data.get("games", [])

            # Extract unique company IDs from game credits
            company_ids = set()
            company_info = {}

            for game in games_found[:20]:
                game_id = game.get("game_id")
                if not game_id:
                    continue

                # Try to get platforms to find company credits
                time.sleep(MOBYGAMES_API_DELAY)
                plat_resp = httpx.get(
                    f"{api_base}/games/{game_id}/platforms",
                    params={"api_key": MOBYGAMES_API_KEY},
                    timeout=30
                )
                if plat_resp.status_code == 200:
                    plat_data = plat_resp.json()
                    platforms = plat_data.get("platforms", [])

                    for plat in platforms:
                        # Check if name appears in attributes (publisher/developer)
                        for attr in plat.get("attributes", []):
                            attr_name = attr.get("attribute_category_name", "").lower()
                            if attr_name in ["developer", "publisher"]:
                                companies = attr.get("attribute_category_value", "").split(",")
                                for comp in companies:
                                    if name.lower() in comp.lower():
                                        company_info[game.get("title", "")] = {
                                            "role": attr_name,
                                            "year": plat.get("first_release_date", "")[:4] if plat.get("first_release_date") else None
                                        }
            ...
            api_data = {
                ...
                "company_name": name,
                "fetch_date": datetime.now().strftime("%Y-%m-%d"),
                "games_data": search_data,
                "company_games": company_info,
                "note": "Data from MobyGames API v1"
            }

            api_file = output_dir / "mobygames_api.json"
            api_file.write_text(json.dumps(api_data, indent=2))
            files_created += 1
            print(f"    Saved MobyGames API data ({len(company_info)} games linked)")

        elif resp.status_code == 429:
            print("    MobyGames API rate limited, skipping")
        else:
            print(f"    MobyGames API error: {resp.status_code}")

    except Exception as e:
        print(f"    MobyGames API error: {e}")
...
def research_wikipedia(name: str, output_dir: Path) -> int:
    ...
    # Try different Wikipedia article name formats
    wiki_names = [
        name.replace(' ', '_'),
...
def research_brave_search(name: str, output_dir: Path) -> tuple[int, list[str]]:
    """Run comprehensive Brave searches for company info."""
    files_created = 0
    discovered_urls = []

    # Expanded query list for companies
    queries = [
        f'"{name}" video game developer history',
        f'"{name}" game studio founded',
        f'"{name}" Sierra On-Line',
        f'"{name}" game company acquisition',
        f'"{name}" video game studio games list',
        f'"{name}" game developer interview',
        f'"{name}" retrospective gaming',
        f'"{name}" game studio closure OR shutdown',
        f'"{name}" notable games developed',
        f'"{name}" game development studio founders',
    ]
...
def research_kagi(name: str, output_dir: Path) -> tuple[int, list[str]]:
    """Query Kagi FastGPT for sourced company facts."""
    files_created = 0
    discovered_urls = []

    queries = [
        f"What video games did {name} develop? List each game with year.",
        f"Tell me the history of {name} game development studio.",
        ...
        f"Who were the key people at {name} game studio?",
    ]
...
def research_additional_sources(name: str, output_dir: Path) -> int:
    """Fetch additional company sources."""
    files_created = 0
...
def research_developer(name: str, force: bool = False) -> dict:
    """Run complete research pipeline for a single developer."""
    slug = slugify(name)
    ...
    # 1. Get vault games for this developer
    vault_games = get_vault_games_for_developer(name)
    ...
    # 2. MobyGames (scrape)
    print("  Phase 1a: MobyGames (scrape)...")
    total_files += research_mobygames(name, output_dir)

    # 2b. MobyGames API
    print("  Phase 1b: MobyGames API...")
    total_files += research_mobygames_api(name, output_dir)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Developer Research Pipeline")
    parser.add_argument('--developer', help='Research a single developer')
    parser.add_argument('--all', action='store_true', help='Research all developers')
    parser.add_argument('--list', help='File with developer names (one per line)')
    ...
    if args.developer:
        research_developer(args.developer, args.force)
    elif args.all:
        results = []
        ...
        if num_workers > 1:
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                futures = {
                    executor.submit(research_developer, name, args.force): name
            ...
            for name in ALL_DEVELOPERS:
                result = research_developer(name, args.force)
                results.append(result)
        ...
        for name in names:
            if name.strip():
                research_developer(name.strip(), args.force)
    else:
        parser.print_help()
