...
tags: [2000s, gobliiins, puzzle, sierra]
---
# Gobliiins 4
...
Gobliiins 4 is the fourth installment in the beloved puzzle-adventure series that began in 1991, marking the franchise's return after a fifteen-year hiatus since Goblins Quest 3 in 1993[^ref-1]. Developed by Société Pollene and produced by the Russian company Snowball Studios, the game was designed by series creator [[Pierre Gilhodes]] alongside [[Muriel Tramis]][^ref-2]. The game returns to the original formula of controlling three goblins with unique abilities, as indicated by the four "i"s in the title—a naming convention that reflects the number of playable characters[^ref-3].
...
> **Designer:** [[Pierre Gilhodes]], [[Muriel Tramis]][^ref-2]
> **Publisher:** Kalypso Media, Snowberry Connection[^ref-8]
> **Engine:** Custom 3D Engine[^ref-2]
...
The Kingdom of King Balderon VIII has been enjoying peace and prosperity for years when disaster strikes—the king's beloved pet, a young aardvark (orycterope) named Riri, has gone missing from the royal palace[^ref-9]. Desperate to recover his favorite companion, the king commissions the renowned Detective Tchoup and his two associates to track down the escaped pet. Young Riri can only hope that the three Gobliiins will solve the riddles before him and ultimately save both him and his fiancée[^ref-10].

The player controls three distinct goblin characters, each with their own unique personality and limited skillset. Tchoup serves as the brain of the operation, investigating clues and gathering information from the various characters encountered throughout their journey[^ref-11]. Stucco provides the muscle, able to grab, handle, and manipulate physical objects that require strength[^ref-12]. Perluis rounds out the trio as the magic user, possessing the ability to make plants grow and bewitch objects with his mystical powers[^ref-8].

True to goblin nature, the three protagonists are not especially bright, and each is capable of performing only one type of action[^ref-8]. This limitation forms the core of the gameplay, as players must strategically combine the talents of all three goblins to overcome obstacles and progress through the adventure. The story unfolds across sixteen levels as the trio travels through increasingly bizarre and challenging environments in pursuit of Riri[^ref-13].

## Gameplay

### Interface and Controls

Gobliiins 4 employs a point-and-click interface that allows players to switch between the three goblin characters at will[^ref-14]. The game uses a simple cursor-based interaction system where players select a goblin and then click on objects or characters within the environment to perform that goblin's unique action. Unlike modern adventure games, the title does not feature an integrated hint system—a significant omission that led one reviewer to note "If there is a game that ever needs a hint system, this game will be it!"[^ref-5].

The game utilizes an archaic save system that provides only a single save slot per level, relying on a password code system rather than traditional save files[^ref-15]. This design choice was criticized for feeling outdated, with one reviewer describing it as "very archaic; from the sub-par graphics, the level-by-level gameplay, even the codes used instead of a save file"[^ref-15]. The intrusive auto-save feature and limited save options were noted as sources of frustration[^ref-16].

### Structure and Progression

The game is structured around sixteen self-contained levels, each consisting of a single screen packed with interactive elements and puzzles[^ref-13]. This level-based approach was compared to "a point and click version of The Lost Vikings," requiring players to combine the three siblings' talents to find solutions[^ref-16]. Players must complete all puzzles within a level before receiving a code to access the next area.

- **Levels 1-13:** Main story progression with hidden golden teeth collectibles[^ref-14]
- **Levels 14-15:** Advanced puzzle areas
- **Bonus Level:** Unlocked by collecting all golden teeth from the first thirteen levels[^ref-14]

The game compels repetition, forcing players to replay levels if certain actions are not completed correctly[^ref-14]. This design philosophy reflects the series' classic approach but proved divisive among modern players accustomed to more forgiving game structures.

### Puzzles and Mechanics

The core gameplay revolves around solving multi-stepped puzzles using the unique abilities of each goblin character[^ref-14]. Tchoup's investigative skills allow him to speak with NPCs and gather crucial information—sometimes requiring multiple conversations with the same character[^ref-17]. Stucco's strength enables physical manipulation of objects, while Perluis's magic can transform and enchant elements of the environment.

The puzzle solutions were criticized for being "annoyingly strait-jacketed at times," requiring very specific sequences of actions[^ref-6]. Trial and error became a common approach, with one reviewer noting "I don't mind hard puzzles, but I hate it when the player is reduced to just using trial and error to progress"[^ref-18]. However, fans of the series argued that "every puzzle is reasonable" and encouraged players to attempt solving them independently[^ref-11].

A notable feature inherited from the original game is the goblins' extraordinary vitality—in other words, they can never die[^ref-19]. This removes the threat of game-over scenarios but does not diminish the challenge of the puzzles themselves.
...
Just Adventure awarded the game a remarkable 91/100, declaring it "a cleverly designed and entertaining game"[^ref-6]. Adventure Gamers gave it 80/100, calling it "one of the most likeable titles this year" and noting that "unless you play adventures only for their stories, I can safely say that this game is worth the time of any point-and-click adventurer"[^ref-6]. GameStar provided a mixed 68/100, observing "The series continues after 15 years and it's true to its predecessors. That's good and bad at the same time. It's a colorful slapstick adventure, but today a game like this needs a real story, characters and some more functions"[^ref-6].

Adventure Classic Gaming rated the game 2/5 (Fair), with reviewer Martin Mulrooney concluding "Sadly, for other adventure game fans, a recommendation cannot be offered with a clear conscience"[^ref-5]. GameGrin awarded 5/10, criticizing the disconnect between the child-friendly presentation and adult-level puzzle difficulty[^ref-15]. LEVEL (Czech Republic) gave 50/100, describing it as a "sequel to the famous trilogy changed into a kid's entertainment game with uncomplicated logical puzzles. The sound and graphics departments should be fired anyways"[^ref-6].

Total PC Gaming scored it just 30/100, summarizing "Gobliiins 4 has a lot of character, it just doesn't have enough depth"[^ref-6]. PC Gamer UK was particularly harsh, awarding only 11/100 with the dismissive assessment "Deliberately obtuse is just frustrating"[^ref-6].
...
**Aggregate Scores:**
- **GameRankings:** 56.5% (based on multiple reviews)[^ref-2]
- **Metacritic:** 54 (based on 13 critic reviews)[^ref-6]
- **IMDB:** 5.5/10[^ref-20]
- **MobyGames:** 6.3/10[^ref-21]
...
Pierre Gilhodes returned to game development after a decade of absence to resurrect his beloved franchise[^ref-23]. Having created the original Gobliiins characters alongside [[Muriel Tramis]] at Coktel Vision in the early 1990s, Gilhodes maintained a vision for continuing the series[^ref-24]. The long gap between Goblins Quest 3 (1993) and this fourth installment represented one of the longest hiatuses in adventure gaming history for a returning franchise.

Gilhodes continued working as a graphic artist and author of animations for various audiovisual productions during the intervening years before launching the production of Gobliiins 4[^ref-25]. His artistic background since a very early age informed the game's distinctive visual style, which aimed to preserve the whimsical aesthetic that defined the series[^ref-25].

### Production

To bring Gobliiins 4 to life, Gilhodes teamed up with Société Pollene, a small studio from the south of France[^ref-13]. Financial backing came from Snowberry Connection, an independent production fund based in Moscow[^ref-13]. This international collaboration between French creative talent and Russian production resources enabled the project to move forward despite the franchise's long dormancy.

The development embraced 3D graphics technology while consciously maintaining the gameplay mechanics that had defined the series[^ref-2]. The decision to preserve the classic formula was deliberate, though it would later draw criticism for feeling outdated. The game was designed to be playable by all ages, with a "kidults style" aesthetic that Gilhodes personally championed[^ref-13].

**Development Credits:**[^ref-25]
- **Designer:** Pierre Gilhodes
- **Co-Designer:** Muriel Tramis
- **Music:** Didier Sallustro[^ref-8]
- **Additional Development:** Vincent L'Hermite, Christian Laly[^ref-25]

### Technical Achievements

The transition to 3D graphics represented the most significant technical evolution from the original trilogy[^ref-2]. However, reviews were divided on whether this constituted an improvement, with some critics finding the graphics "sub-par" despite the technological upgrade[^ref-15]. The game maintained the single-screen level design of its predecessors while rendering environments and characters in three dimensions.

The game supported an impressive array of languages for localization: English, French, Italian, German, Spanish, Russian, Polish, Czech, and Hungarian[^ref-26]. This extensive localization effort reflected the franchise's international appeal and the publishers' commitment to reaching global audiences.

### Technical Specifications

**Minimum Requirements:**[^ref-14]
- **Operating System:** Windows XP/Vista
- **Processor:** Pentium 4 1.8 GHz (1.0 GHz minimum noted in some sources)[^ref-5]
- **RAM:** 512 MB - 1 GB
...
The game shipped on a single DVD-ROM disc[^ref-5]. The copy protection system employed ProtectDiSC v9 technology[^ref-26].

### Technical Issues

The game exhibited stability problems, being noted as "prone to crashing" by reviewers[^ref-16]. Compatibility issues emerged with newer operating systems, with users reporting that the game would install correctly but fail to launch—appearing in task manager without actually running[^ref-28]. The buggy translations from French to English also drew criticism[^ref-5].

The limited save system proved particularly frustrating, offering only a single save per level through an intrusive auto-save mechanism or password system[^ref-16]. This archaic approach stood in stark contrast to modern gaming conventions and contributed to player frustration.

### Version History

| Version | Date | Platform | Notes |
|---------|------|----------|-------|
| 1.0 | March 2009 | Windows | Initial release (MULTI5)[^ref-26] |
| 1.0 | April 2009 | Windows | MULTI3 release[^ref-26] |
| 1.01 | 2009 | Windows | Patch release with language updates[^ref-29] |
| Platinová Edice | April 16, 2009 | Windows | Czech platinum edition[^ref-21] |

**Available Patches:**[^ref-29]
- Czech Patch (573KB)
...
- Hidden golden teeth are concealed on each screen; collecting all teeth from the first thirteen levels unlocks a secret bonus level[^ref-5]
- The number of "i"s in the series titles directly corresponds to the number of playable goblin characters: Gobliiins (3 goblins), Gobliins 2 (2 goblins), Goblins 3 (1 goblin), Gobliiins 4 (3 goblins)[^ref-3]
- The US release of the original Gobliiins renamed the characters to Hooter, Dwayne, and BoBo, whereas European versions used Oups, Ignatius, and Asgard[^ref-30]
- Pictures of previous games' goblins appear in scenes throughout the series[^ref-3]

## Legacy

### Sales and Commercial Impact

Gobliiins 4 launched at a retail price of £19.99 in the UK and was available for digital download through GamersGate[^ref-8]. The game's commercial performance was modest, reflecting its niche appeal to fans of classic adventure gaming. One reviewer suggested that "this is a full price retail game, yet I think it would have worked far more effectively as a cheaper, digital download"[^ref-18].

The game's distribution became increasingly limited over time, with the title becoming notably difficult to acquire. Community members on GOG.com's wishlist page lamented that "a lot of people do not even know this game exists"[^ref-22], highlighting the franchise's fading visibility in the broader gaming market.

### Collections

The game was released in a "Platinová Edice" (Platinum Edition) in Czech territories, which contained additional content[^ref-21]. The original trilogy remained available through various digital storefronts including GOG.com and Steam, often bundled together, though Gobliiins 4 itself proved harder to obtain.

### Fan Projects

The series creator, Pierre Gilhodes, continued developing the franchise independently, releasing Gobliiins 5 (titled "GOBLiiiNS5 - The Invasion of the Morglotons") in July 2023 on Steam[^ref-7]. This sequel was created using Adventure Game Studio and received "Very Positive" reviews (93% of 60 reviews)[^ref-7]. The game was also made available on itch.io for $11 USD[^ref-31].

### Related Publications

- **In-game hints:** Players relied heavily on community-created walkthroughs due to the absence of an official hint system[^ref-11]
- **Online Walkthrough:** Comprehensive text walkthroughs available on GameFAQs, Neoseeker, and other community sites[^ref-11]
...
- [Internet Archive - CD-ROM Image](https://archive.org/details/gobliiins4cdrom)[^ref-32]
- [Internet Archive - Spanish Version](https://archive.org/details/Gobliiins4Spanish)[^ref-33]
- [Internet Archive - Multi-language](https://archive.org/details/gobliiins-4)[^ref-34]
